...
for k=2:1:N
    ...
    %实际观测值
    Z_meas(1,k) = X_real(1,k)^2 + V_noise(1,k);
  %卡尔曼滤波
        %状态预测:X_pre
        ...
        X_pre(k) = diff_X_esti - rema;     %一步状态预测
        ...
        %计算测量预测误差协方差: P_z_pre
        P_z_pre = 1/(4*h_con^2)*( (X_pre(k)+h_con*S_chol_pre)^2 - ...
                  (X_pre(k)-h_con*S_chol_pre)^2 )^2 + R;

        %计算测量预测误差 互协方差: P_xz_pre
        P_xz_pre = 1/(2*h_con)*S_chol_pre*( (X_pre(k)+h_con*S_chol_pre)^2 - ...
                  (X_pre(k)-h_con*S_chol_pre)^2 );

        %计算卡尔曼增益:Kk(2*1)
        Kk = P_xz_pre/P_z_pre;

        %状态更新
        X_esti(1,k) = X_pre(k) + Kk*( Z_meas(1,k) - X_pre(k)^2 );
