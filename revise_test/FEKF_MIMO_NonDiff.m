...
U_input = idinput(N,'sine',[0,1],[-1,1])';
%U_input = zeros(1,N);
% U_input = ones(1,N);            
% for i = N/2+1:1:N
%     U_input(:,i) = -2;
...
% k = 1:1:N;
% figure;
% plot(k,X_real(1,:),'b',k,X_real(2,:),':r',k,X_real(3,:),'-.g','linewidth',LineWidth);
% legend('实际状态1','实际状态2','实际状态3','Location','best');

for k=2:1:N
  %卡尔曼滤波
        %状态预测:X_pre
        diff_X_esti = [cos(X_esti(2,k-1)); ...
               -0.1*X_esti(2,k-1)+ exp(-0.05*X_esti(3,k-1))+U_input(1,k-1); ...
               -X_esti(3,k-1)-0.5*abs(X_esti(1,k-1))]; %-X_esti(2,k-1) 
        %diff_X_esti = [X_esti(2,k-1); -0.1*X_esti(1,k-1)-X_esti(2,k-1)*X_esti(3,k-1)+U_input(1,k-1); 0];
            %计算余项
            rema = [0;0;0];
            if k>L
                for i = 2:1:L+1
        ...
        %预测误差协方差矩阵:P_pred
            ...
            F = [ 0, -sin(X_esti(2,k-1)), 0; ... 
                  0, -0.1, -0.05*exp(-0.05*X_esti(3,k-1)); ...
                -0.5*X_esti(1,k-1)/abs(X_esti(1,k-1)), 0, -1 ]; %-0.05*exp(-0.05*X_esti(3,k-1))
        P_pred = (F-gamma{1,2})*P_esti{1,k-1}*(F-gamma{1,2})'+ Q + rema_P;
        %计算卡尔曼增益:Kk(2*1)
        H = [0.1,0.3,0];
        Kk = P_pred*H'/(H*P_pred*H' + R);
        %状态更新
        X_esti(:,k) = X_pre + Kk*( Z_meas(k) - 0.1*X_pre(1,1) - 0.3*X_pre(2,1) );
        %估计方差矩阵更新
