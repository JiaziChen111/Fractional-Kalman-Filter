...
%         同时对系统噪声均值以及测量噪声均值进行估计效果并不好：虽然状态估计效果
%         较好，但是噪声均值估计不同时收敛，而且两噪声均值收敛至两实际数据的中间
%         值，并稍微向自身真实值靠拢。原因猜测为两个噪声有耦合，该方法无法分清。
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for k=2:1:N
  ...
  %卡尔曼滤波
        ...
        X_pre = diff_X_esti - rema + q_esti(k-1);     %一步状态预测
        %预测误差协方差矩阵:P_pred
...
figure;
plot(k,q_esti(1,:),'b',k,r_esti(1,:),'r','linewidth',LineWidth);
legend('系统噪声均值收敛趋势线','测量噪声均值收敛趋势线','Location','best');
line([0,N],[r_con,r_con],'lineStyle','--')
line([0,N],[q_con,q_con],'lineStyle','--')
