...
err_P = zeros(2,N);             %均方误差
err_P(1,1) = P_esti{1,1}(1,1);
err_P(2,1) = P_esti{1,1}(2,2);
...
for k=2:1:N
    ...
    rema = [0;0];
    for i = 1:1:k-1
        rema = rema + ((-1)^i)*gamma{1,i+1}*X_real(:,k-i);
    end
    ...
    %卡尔曼滤波
        ...
        diff_X_esti = A*X_esti(:,k-1) + B*U_input(k-1);
            ...
            if k>L
                for i = 1:1:L
                   rema = rema + ((-1)^i)*gamma{1,i+1}*X_esti(:,k-i);
                end
            else
               for i = 1:1:k-1
                   rema = rema + ((-1)^i)*gamma{1,i+1}*X_esti(:,k-i);
    ...
    X_pre = diff_X_esti - rema;
    %预测误差协方差矩阵:P_pred
        %计算余项
        rema_P = [0,0;0,0];
        if k>L
            for i = 3:1:L+1
                rema_P = rema_P + gamma{1,i+1}*P_esti{1,k+1-i}*gamma{1,i+1}';
            end
        else
           for i = 3:1:k
    ...
    %计算均方误差
    temp_matrx = P_esti{1,k};
    err_P(1,k) = temp_matrx(1,1);
    err_P(2,k) = temp_matrx(2,2);
end

% figure;
% plot(X_real(1,:),'linewidth',LineWidth);
% hold on
% plot(X_real(2,:),'r','linewidth',LineWidth);
